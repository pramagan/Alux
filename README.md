...
So each check — whether triggered by "Check now" or the periodic alarm — runs up to two calls:

1. **Jev decides if anything's worth mentioning — and classifies every video while it's at it.** `checkYoutubeHistory()` in `background.js` builds a request via `watch.buildJevDecisionRequest()` — your instruction and the recent watch titles go into `state`, a `should_flag` question (type `noul`) asks Jev whether the history as a whole matches what you described, and one more `noul` question is added *per video* so each entry gets its own match/no-match classification in the same call. This is fast and cheap (Jev is priced near-zero output cost) and runs on **every** check.
2. **If Jev's confidence is > `FLAG_THRESHOLD` (0.8, in `lib/watch.js`), a second call writes the actual note.** `watch.buildWriteupMessages()` builds a normal chat-completions prompt (same instruction + history, plus a reaction note — see below) and sends it to whatever model is in the "model" field, via `openrouter.sendChatMessage()`. This step is skipped — no second API call, no extra cost — when Jev doesn't flag anything; the popup just shows a plain "nothing stood out" message instead.
