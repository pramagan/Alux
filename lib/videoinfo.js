const EMPTY_INFO = { title: '', description: '', channelTitle: '' };
...
// player to read the title, description, and channel name. This is not a
// sanctioned API: YouTube can change its page structure at any time and
// silently break this, and scraping it programmatically may not comply with
// YouTube's terms of service. Every failure mode (network error, missing
// blob, JSON shape change) degrades to empty strings rather than throwing,
// so one bad video never blocks the rest of a check.
...
// Jev still gets a title and video id to classify on. The already-known
// chrome.history title wins over the freshly-scraped one (spread order),
// since it's what the browser actually recorded and is virtually always
// present; the scraped `title` mainly exists for callers with no history
// entry to begin with — see evals/run.js.
export async function enrichEntriesWithPageInfo(entries) {
  ...
  return entries.map((entry, i) => ({ ...infos[i], ...entry }));
}
