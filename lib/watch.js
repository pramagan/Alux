...
// one entry per video (sorted most-recent-first). This only sees what
...
//
// Each entry carries `visitCount`: how many times *within this window* that
...
// history, not scoped to lookbackMs, so it would conflate "watched 5 times
...
// and filtered down to ones inside the window, so the count reflects actual
// rewatching during this check — a real doomscrolling signal that collapsing
// to one row per video would otherwise silently discard.
export async function queryYoutubeHistory(lookbackMs = DEFAULT_LOOKBACK_MS) {
  const startTime = Date.now() - lookbackMs;
  const results = await chrome.history.search({ text: 'youtube.com', startTime, maxResults: 1000 });
  ...
  await Promise.all(
    entries.map(async (entry) => {
      ...
      const recentVisitCount = visitLists.flat().filter((visit) => (visit.visitTime || 0) >= startTime).length;
