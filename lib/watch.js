...
// history — no cleanup beyond a time window and a hard cap on the total. No
// cursor, no de-duplication of repeat visits to the same video: whatever
// chrome.history.search finds within that window is what gets returned, one
// entry per matching history item. Two limits: HISTORY_LOOKBACK_MS bounds
// how far back the search itself goes (the past month — searching all-time
// history isn't useful for "am I doomscrolling this lately," and it keeps
// the raw search cheap), and MAX_VIDEOS caps the total video entries
// returned (the most recent 1000), so a very long-lived profile's history
// doesn't grow the Jev batching loop in background.js without bound.
//
// The raw chrome.history.search call asks for more than MAX_VIDEOS
// (RAW_SEARCH_LIMIT) because most youtube.com history hits aren't actual
// videos (homepage visits, search-results pages, channel pages) — asking for
// only 1000 raw items could leave well under 1000 real videos after
// filtering even if 1000+ actually exist within the window.
//
...
export async function queryYoutubeHistory() {
  const windowStart = Date.now() - HISTORY_LOOKBACK_MS;
  ...
  console.log(
    `[Alux] queryYoutubeHistory: rawResults=${results.length} (searched since ${new Date(windowStart).toISOString()}, past 30 days).`
  );
  ...
  const capped = entries.slice(0, MAX_VIDEOS);
  console.log(
    `[Alux] queryYoutubeHistory: ${entries.length} video entries found, ${capped.length} kept after capping to ${MAX_VIDEOS}.`
  );
...
// just leaning yes. It feeds into each batch's match rate, which in turn
// feeds STRIKE_MATCH_THRESHOLD below — flagging/interruption is now driven
// by strikes (see checkYoutubeHistory() in background.js), not by comparing
// an aggregate match rate against this constant directly.
export const FLAG_THRESHOLD = 0.8;

// Per-batch bar for strike-counting (see checkYoutubeHistory() in
// background.js) — deliberately lower than FLAG_THRESHOLD, since it's
// judging a whole ~50-video chunk's match rate, not one video's confidence.
// Any batch whose match rate exceeds this earns a strike, and strikes (not
// an aggregate match rate) are what now gate the pause/voice/redirect
// interruption.
export const STRIKE_MATCH_THRESHOLD = 0.7;
