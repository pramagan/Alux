...
export async function queryYoutubeHistory(windowMs, sinceTimestamp = 0) {
  // chrome.history.search returns results most-recent-first, so bounding by
  // sinceTimestamp alone (no upper bound) is enough to reliably find the
  // true most recent YouTube watch, however long ago that actually was.
  ...
  if (anchorTime === 0) return []; // nothing watched since the cursor at all
