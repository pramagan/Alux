...
- **Each flagged-window video's public watch page.** To classify more accurately than a bare title allows, Alux fetches the public watch page for each video in that short window (the same page any signed-out visitor could load) and extracts its description and channel name — this means a request is made to `youtube.com` for each video being checked, and that description/channel name is also sent to OpenRouter alongside the title. No YouTube account, cookies, or API key are used for this; it's the same page anyone can already see.
